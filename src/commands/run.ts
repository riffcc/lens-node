...
import { setupSubscriptionSync, federateNewContent as federateNewContentWrapper, setupSyncMonitoring } from './run-sync-wrapper.js';
    ...
    try {
      ...
      site.subscriptions.events.addEventListener('change', async (evt) => {
        ...
        if (addedCount > 0 && evt.detail.added) {
          try {
            // Sync is now handled internally by lens-sdk's FederationService
            statusManager.showMessage(`🔄 Setting up sync for ${addedCount} new subscription(s) added via UI...`);
            // The FederationService will automatically handle sync when subscriptions are added
      ...
      try {
        ...
        if (initialSubs.length > 0) {
          ...
          // Sync is now handled internally by lens-sdk's FederationService
          await setupSubscriptionSync(client!, site, lensService, initialSubs, statusManager);
        }
      } catch (error) {
...
async function restoreDeletedContent(lensService: LensService) {
  try {
    ...
    try {
      // Always open the site for content restoration
      ...
      const shouldCloseAfter = true; // Always close after we're done
